...
import { API_CREATE_BOOKING, API_GET_ALL_DOCTOR_BY_CLINIC, API_GET_ALL_SERVICES_OF_CLINIC, API_GET_CLINIC_BY_ID } from "@/constants/api"
...
export const createBooking = async (slot: number, type: number, bookingDate: Date, customerId: number, clinicId: number, serviceId: number) => {
    try {
        const response = await axiosInstance.post(`${API_CREATE_BOOKING}`,{
            slot, type, bookingDate, customerId, clinicId, serviceId
        })
        if (response) {
            return response;
        }
