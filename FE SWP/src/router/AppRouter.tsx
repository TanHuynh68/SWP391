...
import {
  AdminDashboard, AdminLogin, ClinicPage, CustomerBookingHistory, CustomerBookingPage, CustomerLayout, Dashboard, Home, InternalServerError, Login, ManageAppointmentSchedule, ManageBooking, ManageClinic, ManageClinicOwner, ManageDoctor, ManageMedicalExaminationSchedule, ManagePatient, ManageUser,
  NotFound,
  RegisterClinic,
  ScheduleOfWeek
} from "../pages";
...
const AppRouter: React.FC = () => {
  ...
  return (
    <Routes>
      ...
      <Route path={paths.CLINIC_DETAIL} element={<DentalDetails />} />
      ...
      <Route path="/customer/*" element={canAccess([role.CUSTOMER]) ? <CustomerLayout /> : <Navigate to={paths.HOME} />}>
        ...
        <Route path={paths.CUSTOMER_CLINIC_DETAIL} element={canAccess([role.CUSTOMER]) ? <DentalDetails /> : <Navigate to={paths.HOME} />} />
      ...
      <Route path="/clinic-owner/*" element={canAccess([role.CLINIC_OWNER]) ? <Dashboard /> : <Navigate to={paths.HOME} />}>
        ...
        <Route path={paths.MANAGE_APPOINTMENT_SCHEDULE} element={<ManageAppointmentSchedule />} />
      </Route>
      {/* Dentist
